...
const Results = () => {
    ...
    try {
        if (snippet.value.length <= 0) {
            return (<div className="linkedInModal--content__right__inputs--results linkedInModal--content__right__inputs--results--err">
        ...
        return (
            <div className="linkedInModal--content__right__inputs--results linkedInModal--content__right__inputs--results--ok" id= {r}>
                <p onClick={() => {navigator.clipboard.writeText(document.querySelector('.linkedInModal--content__right__inputs--results--ok').innerText);}}>Conversion Id: {r}</p>
            </div>
        );
    } catch {
        return (
...
const Modal = () => {
    ...
    return (
        <div className="linkedInModal-container">
                ...
                <div className="linkedInModal--content">
                    <div className="linkedInModal--content__right">
                        ...
                        <div className="linkedInModal--content__right__inputs">
                            <input type="text" />
                            <button type="submit" onClick={() => setShowResults(Results)} >Submit</button>
                        </div>
                        {showResults ? <Results /> : null}
                    </div>
                    <div className="linkedInModal--content__left">
                    </div>
                </div>
            </ReactModal>
        </div >
